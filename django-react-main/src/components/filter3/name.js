 ...
 */
 ...
 change_exp1(){
  ...
  this.state.exp1?(this.setState({result:""})) : (this.setState({result:"ФАИТ,"}));
  this.handlExpChange()
 }
 change_exp2(){
  this.setState(previousState => {
  ...
  this.state.exp2?(this.setState({result2:""})) : (this.setState({result2:"ФБТ,"}));
  this.handlExpChange()
}
 change_exp3(){
  this.setState(previousState => {
  ...
  this.state.exp3?(this.setState({result3:""})) : (this.setState({result3:"ФПТ,"}));
  this.handlExpChange()
}
 change_exp4(){
  this.setState(previousState => {
  ...
  this.state.exp4?(this.setState({result4:""})) : (this.setState({result4:"ПензГТУ,"}));
  this.handlExpChange()
}
change_exp5(){
  this.setState(previousState => {
     ...
     <div className={`exp ${css(styles.group, styles.group_layout)}`}>
       <px-posize track-style='{"flexGrow":1}' x="0px minmax(500px,500px) 0px" y="0px minmax(287px,287px) 0px">
         <div className={css(styles.flex)}>
           {
                    this.state.exp1?(
                      <div className={css(styles.txt_selected, styles.txt1_layout)}onClick={this.change_exp1.bind(this)}>{'ФАИТ'}
                      </div>
                ) : (
                  <div className={css(styles.txt1, styles.txt1_layout)}onClick={this.change_exp1.bind(this)}>{'ФАИТ'}
                  </div>)
            }
           ...
           {
                    this.state.exp2?(
                      <div className={css(styles.txt_selected, styles.txt1_layout)}onClick={this.change_exp2.bind(this)}>{'ФБТ'}</div>
                ) : (
                  <div className={css(styles.txt1, styles.txt1_layout)}onClick={this.change_exp2.bind(this)}>{'ФБТ'}</div>)
            }
           { <hr className={css(styles.rect, styles.rect_layout2)} /> }
           {
                    this.state.exp3?(
                      <div className={css(styles.txt_selected, styles.txt1_layout)}onClick={this.change_exp3.bind(this)}>{'ФПТ'}</div>
                ) : (
                  <div className={css(styles.txt1, styles.txt1_layout)}onClick={this.change_exp3.bind(this)}>{'ФПТ'}</div>)
            }
           <hr className={css(styles.rect, styles.rect_layout3)} />
           {
                    this.state.exp4?(
                      <div className={css(styles.txt_selected, styles.txt5_layout)}onClick={this.change_exp4.bind(this)}>{'ПензГТУ'}</div>
                ) : (
                  <div className={css(styles.txt4, styles.txt5_layout)}onClick={this.change_exp4.bind(this)}>{'ПензГТУ'}</div>)
            }
